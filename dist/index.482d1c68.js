...
})({"95SD2":[function(require,module,exports) {
"use strict";
var HMR_HOST = null;
var HMR_PORT = null;
    ...
    constructor(size){
        ...
        this.initGameTable = ()=>{
            ...
            for(let i = 1; i <= this.size.height; i++){
                const row = [];
                for(let j = 1; j <= this.size.width; j++)row.push(0);
        ...
        this.onClickCellHandler = (event)=>{
            ...
            if (cellHtml.classList.contains('active')) {
                ...
                if (this.startGame === true) this.nextGameTable[cellAddressY][cellAddressX] = 0;
                else this.gameTableCurrent[cellAddressY][cellAddressX] = 0;
            } else {
                cellHtml.classList.add('active');
                if (this.startGame === true) this.nextGameTable[cellAddressY][cellAddressX] = 1;
                else this.gameTableCurrent[cellAddressY][cellAddressX] = 1;
            }
        };
        this.runGame = async ()=>{
                ...
                this.gameTableCurrent.forEach((rowItem, rowIndex)=>{
                    rowItem.forEach((cellItem, cellIndex)=>{
                        ...
                        if (this.gameTableCurrent[cellY][cellX] === 1) {
                            if (activeNeighborhoods.length < 2) this.nextGameTable[cellY][cellX] = 0;
                            else if (activeNeighborhoods.length === 2 || activeNeighborhoods.length === 3) this.nextGameTable[cellY][cellX] = 1;
                            else if (activeNeighborhoods.length > 3) this.nextGameTable[cellY][cellX] = 0;
                        } else if (activeNeighborhoods.length === 3) this.nextGameTable[cellY][cellX] = 1;
                    });
                });
        ...
        this.getNeighborhood = (y, x)=>{
            const topLeft = y === 0 || x === 0 ? null : [
                y - 1,
                x - 1
            ];
            const top = y === 0 ? null : [
            ...
            const topRight = y === 0 || x === this.size.height - 1 ? null : [
                y - 1,
                x + 1
            ];
            const right = x === this.size.width - 1 ? null : [
                y,
                x + 1
            ];
            const left = x === 0 ? null : [
            ...
            const bottomLeft = y === this.size.height - 1 || x === 0 ? null : [
                y + 1,
                x - 1
            ];
            const bottom = y === this.size.height - 1 ? null : [
                y + 1,
                x
            ];
            const bottomRight = y === this.size.height - 1 || x === this.size.width - 1 ? null : [
                y + 1,
                x + 1
            ];
            return [
...
},{}]},["95SD2","j6eqU"], "j6eqU", "parcelRequire2d9a")
